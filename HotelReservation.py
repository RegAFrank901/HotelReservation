...
while again.lower() == "y":
    # get arrival date
    while True:
        date_str = input("Enter arrival date (MM-DD-YYYY): ")
        try:
            arrival_date = datetime.strptime(date_str, "%m-%d-%Y")
        except ValueError:
    ...
    # get departure date
    while True:
        date_str = input("Enter departure date (MM-DD-YYYY): ")
        try:
            departure_date = datetime.strptime(date_str, "%m-%d-%Y")
        except ValueError:
            print("Invalid date format. Try again.")
            print()
            continue  # skip next if statement and re-start loop
